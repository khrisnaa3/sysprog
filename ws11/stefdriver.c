...
#define  DEVICE_NAME "stefdriver"    // /dev/<value>
#define  CLASS_NAME  "stefdrc"          // The device class

MODULE_LICENSE("GPL");            
MODULE_AUTHOR("Stefanus");    
...
static int dev_open(struct inode *inodep, struct file *filep){
   numberOpens++;
   printk(KERN_INFO "stefdriver: Device has been opened %d time(s)\n", numberOpens);
   return 0;
}

static int dev_release(struct inode *inodep, struct file *filep){
   printk(KERN_INFO "stefdriver: Device successfully closed\n");
   return 0;
}
...
static int __init rdchardrv_init(void){
   ...
   printk(KERN_INFO "stefdriver: device class created correctly\n");
   return 0;
}
...
static void __exit rdchardrv_exit(void){
    ...
    class_destroy(classptr);                              
    unregister_chrdev(majorNumber, DEVICE_NAME);              
    printk(KERN_INFO "stefdriver: Goodbye from the STEF!\n");
}
